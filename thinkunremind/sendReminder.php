...
<div class="wrap">	
	...
	<div style="float:right;">
		...
		<p style="padding-top:5px;"><a class="link-button" style="padding-right:12px;" href="<?php echo get_bloginfo('url')?>/wp-content/plugins/thinkunremind/exportData.php?dirPath=<?=dirPath?>">Export logs as CSV</a></p>
	</div>
					...
					<div id="titlediv">
						...
						<?php 
							...
							foreach($this->getCustomField() as $key=>$cField) 
							{
								...
								$customField .= '|'.$cField->field_code;
						?>
						<div class="lable"><?=$cField->field_name?></div>
						<div class="field">
							<input type="text"  class="input-box" value=''  name="customField[<?=$cField->field_code?>]" id="<?=$cField->field_code?>">
						</div>
						<div style="clear:both"></div>
						<?php 
							} 
	...
	<!--- Create form with target to iframe: For Preview -->
		...
		<form id="frmPreview" name="frmPreview" method="post" target="previewIframe" action="<?= plugins_url('thinkunremind/emailPreview.php')?>">
